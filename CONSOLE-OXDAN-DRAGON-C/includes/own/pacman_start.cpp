	...
	while ((*LD.lives > 0) && (*LD.dotsEaten < *LD.maxDots)) {
		...
		*(LD._level[*_pY][*_pX]) = pac;
		...
		if (kbhit()) {
			...
			case 's': pac = '^'; break;
			case 'd': pac = '<'; break;
			case VK_UP: pac = 'v'; break;
			case VK_LEFT: pac = '>'; break;
			case VK_DOWN: pac = '^'; break;
			case VK_RIGHT: pac = '<'; break;
			}
		}
...
int titleScreen() {// Displays title screen.
	...
	setConsoleColour(2);
	cout << "\033[0;33mNew game starting...\033[0;37m\nSelect level (1 or 2): ";
	int lvl;
	cin >> lvl;
	return lvl;
}
...
void pacman_start()
{
	system("mode 77, 24"); // set size of console (+)
	...
	char in = 'r';
	while (in == 'r') // while
	{ ///While the user keeps entering 'r' for retry at the end of each round, game loop does not stop.
