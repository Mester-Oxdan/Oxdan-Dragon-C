...
RealPlayer_checkers::RealPlayer_checkers(bool topSideOfBoard, CheckersBoard_checkers*board, Button_checkers*buttons): Player_checkers(topSideOfBoard, board, buttons){
    selectingState = false;
}

RealPlayer_checkers::~RealPlayer_checkers(){
    delete Board;
    Board = NULL;
    delete boardButtons;
    boardButtons = NULL;
...
bool RealPlayer_checkers::makeMove(SDL_Event* event){
...
void RealPlayer_checkers::selectPiece(int x, int y){
    // SELECT PIECE //
...
bool RealPlayer_checkers::selectedLocationIsValid(int currentIndex, int x, int y, bool forHighlight) {
...
void RealPlayer_checkers::highlightValidMoves() {
    int x, y;
    for (int i = -2; i <=2; i++){
